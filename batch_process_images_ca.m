function batch_process_images_ca(directory_path)
...
function num_cells = process_image(FILENAME)
    ...
    % Keep red channel
    I_cell = I(:,:,2);  % Green Channel is channel 1
    ...
    bw = imbinarize(I_filtered, 'adaptive', 'ForegroundPolarity', 'bright', 'Sensitivity', 0.0000001); %% For CA images 

    % Remove small objects (noise)
    min_cell_area = 100;  % Adjust based on expected cell size 300 for CA images 
    ...
    mask = imextendedmin(D, 2);  % Adjust the H-minima transform height
